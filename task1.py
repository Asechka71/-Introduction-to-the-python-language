...
def lottery(people1,people2):    
    while True:
        ...
        if people2 > people1:
            return 'people2' 
            break
        print('Выпала ничья, проходит жеребьёвка')
...
while candy > 0:
    ...
    if player == 'people1':
            a =(int(input('Сколько конфет берет первый игрок? ')))
            if 0<a<=28:
                candy = candy - a 
                player = 'people2'
            else: print('Введено неверное количество конфет! За один ход можно взять небольше 28 конфет!')    
    else:
